...
function load() {
 ...
 var headername = localStorage.getItem("headername");
    ...
    try {
        // alert(ip);
        $.ajax({
            url: 'http://' + ip + '/Slim_Proxy_okulsis/SlimProxyBoot.php?url=OgrenciKarnesi_mbllogin&donemID=' + gelendonem + '&ogrenciID=' + kisiid + '&cid=' + cid + '&lid=' + lid + '&did=' + did + '',
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                var j;
            ...
            $.ajax({
                ...
                success: function (data) {
                  ...
                  /*  document.getElementById('hds').innerHTML = sum;
                    ...
                    document.getElementById('atoplam2').innerHTML = atoplam2;
                    document.getElementById('aortalama2').innerHTML = aortalama2;*/
                    $('#toplam').append('<tr><td>' + sum + '</td><td class="hs">' + atoplam1 + '</td><td>' + aortalama1 + '</td><td>' + atoplam2 + '</td><td>' + aortalama2 + '</td><td>' + daltoplam + '</td></tr>');    
                    $('#degerlendirme').append('<tr><td>' + basaribelgesi + '</td><td class="hs">' + puandegerlendirme + '</td>');    
                }
            });
        } catch (e) {
